import React from 'react'
import './Courses.css'

function Courses() {

  return (

    <div className='courses_outer_shell'>

            <div className='courses_header_background'>
                    <h2>Relational Courses</h2>
            </div>
            <div className='courses_header_image_container'>
                <img className='courses_header_image' src='https://dev.relationalearning.com/wp-content/uploads/2024/03/matt-duncan-IUY_3DvM__w-unsplash.jpg' alt='' />
            </div>

    <div className='courses_mid_page'>

            <div className='teaching_with_relational_mind'>
                <div className='teaching_with_relational_mind_top'>
                    <h3>Teaching with the Relational Mind</h3>
                    <p>Relational work is very personal, and every individual will work relationally in a different and subjective way. This course will help you bring your authentic self to your teaching. Discovering who you are relationally can help you then foster that discovery in others. Participants will engage in an in-depth analysis of their personal relational mindset, as they gain knowledge on how to practice and maintain this. A relational mindset will help any educator increase the engagement of learners, families, and everyone within a school’s orbit.</p>
                    </div>
            <div class='teaching_relational_mind_iframe_container'>
            <iframe src="https://www.learndesk.us/class/4592070314426368/embed" title="Teaching with the Relational Mind" allow="autoplay; fullscreen" ></iframe>

                    <div className='teaching_relational_mind_text'>
                                <h5>Teaching with the Relational Mind</h5>
                                <span>by Dr. George Otero</span>
                                <br></br>
                                <br></br>
                                <p>Duration: 7hrs</p>
                                <p>Included: 1 video, 9 documents, 1 quiz, 9 lessons and 8 assignments</p>
                                <p>Level: Beginner</p>
                                <p>Pricing: $99.99 </p>
                                <p>(90 day access)</p>
                        <a href='https://www.learndesk.us/checkout/single?id=5552815910944768&currency=USD'>
                            <button className='teaching_relational_mind_button'>Buy Now</button>
                        </a>
                    </div>
                </div>
        </div>



        <div className='leading_with_relational_mind'>
                <div className='leading_with_relational_mind_top'>
                    <h3>Leading with the Relational Mind</h3>
                    <p>Participants in this course will learn about the values of fostering healthy relationships, utilizing our unique discovery process that builds positive relationships between families, schools, and communities. New strategies for when and how to foster those healthy leader-learner relationships will be shared and resources for leaders to use in the future will be provided.</p>
                    </div>
            <div class='leading_relational_mind_iframe_container'>
            <iframe src="https://www.learndesk.us/class/5982348278497280/embed" title="Leading with the Relational Mind" allow="autoplay; fullscreen"></iframe>
                    <div className='leading_relational_mind_text'>
                                <h5>Leading with the Relational Mind</h5>
                                <span>by Dr. George Otero</span>
                                <br></br>
                                <br></br>
                                <p>Duration: 7hrs</p>
                                <p>Included: 16 documents, 8 lessons and 14 assignments</p>
                                <p>Level: Beginner</p>
                                <p>Pricing: $99.99 </p>
                                <p>(90 day access)</p>
                        <a href='https://www.learndesk.us/checkout/single?id=6462099075104768&currency=USD'>
                            <button className='leading_relational_mind_button'>Buy Now</button>
                        </a>
                    </div>
                </div>
        </div>






        <div className='living_with_relational_mind'>
                <div className='living_with_relational_mind_top'>
                    <h3>Living with the Relational Mind</h3>
                    <p>Participants in this course will immerse themselves in readings, discussions, as well as hands-on assignments that will start them down a new pathway. Utilizing relational strategies in everyday life is sure to produce positive change in the lives of anyone who gives it an honest try!</p>
                    </div>
            <div class='living_relational_mind_iframe_container'>
            <iframe src="https://www.learndesk.us/class/5555669782495232/embed" title="Living With the Relational Mind" allow="autoplay; fullscreen"></iframe>
                    <div className='living_relational_mind_text'>
                                <h5>Living with the Relational Mind</h5>
                                <span>by Dr. George Otero</span>
                                <br></br>
                                <br></br>
                                <p>Duration: 7hrs</p>
                                <p>Included: 11 documents, 1 quiz, 12 lessons and 11 assignments</p>
                                <p>Level: Beginner</p>
                                <p>Pricing: $99.99 </p>
                                <p>(90 day access)</p>
                        <a href='https://www.learndesk.us/checkout/single?id=4508413344088064&currency=USD'>
                            <button className='living_relational_mind_button'>Buy Now</button>
                        </a>
                    </div>
                </div>
        </div>




        






            </div>

    </div>

  )
}

export default Courses